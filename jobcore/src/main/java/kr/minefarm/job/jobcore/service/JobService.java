...
public final class JobService {
     ...
     * 온라인이면 일반 플로우(onSelect/onDeselect 훅 호출), 오프라인이면 DB만 업데이트.
     */
    public CompletableFuture<Boolean> changeJobByUuid(java.util.UUID targetUuid, JobId newJobId) {
        CompletableFuture<Boolean> done = new CompletableFuture<>();
        profileService.loadOrCreate(targetUuid).whenComplete((profile, error) -> {
    ...
    private void applyJobChangeOffline(PlayerJobProfile profile, JobId newJobId, CompletableFuture<Boolean> done) {
        ...
        saveAndComplete(profile, done);
    }

    private void saveAndComplete(PlayerJobProfile profile, CompletableFuture<Boolean> done) {
